...
<div class="centercontent tables">
		<div class="pageheader notab">
			<h1 class="pagetitle20">编辑菜单</h1>
			<span class="pagedesc">编辑菜单</span>
		</div><!--pageheader-->
		<div id="contentwrapper" class="contentwrapper">
		   <div id="basicform" class="subcontent">
			<form class="stdform formvalidate" action="<?php echo U('Wechat/Admin/menuedit');?>" method="post">
				<p><label>上级菜单</label>
				<span class="field">
					<select name='pid' class="selectwidth40-1">
						<option value='0'>顶级菜单</option>
						<?php if(is_array($menu)): $i = 0; $__LIST__ = $menu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value='<?php echo ($vo["id"]); ?>' <?php if($vo['id'] == $data['pid']): ?>selected<?php endif; ?> ><?php echo ($vo["cat_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
					</select>
				</span></p>
				<p><label>菜单名称</label>
				<span class="field">
				...
				<p><label>排序</label>
					<span class="field">
					<input type="text" name="sort_order" id="sort_order" class="smallinput" value='<?php echo ($data["sort_order"]); ?>' validate='{required:true,messages:{required:"不能为空"}}' />
				</span></p>
				<p><label>动作类型</label>
					<span class="field">
					<input type="radio" name='weixin_type' id="weixin_type" value='0' <?php if($data[weixin_type] == 0): ?>checked<?php endif; ?> />点击
					<input type="radio" name='weixin_type' id="weixin_type" value='1' <?php if($data[weixin_type] == 1): ?>checked<?php endif; ?>/>查看
				
				</span></p>
				<p><label>Key值或URL</label>
					<span class="field">
					<textarea name='weixin_key_links' id="weixin_key_links" rows="10" cols="40"><?php echo ($data["weixin_type_value"]); ?></textarea>
				</span></p>
