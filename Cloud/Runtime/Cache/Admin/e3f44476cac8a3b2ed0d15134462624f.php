 ...
 <a class="togglemenu"></a>
       ...
       <div class="centercontent tables">
        <div class="pageheader notab">
            <h1 class="pagetitle20">短信接口</h1>
            <span class="pagedesc">短信接口的设置</span>
        </div><!--pageheader-->
        
        <div id="contentwrapper" class="contentwrapper">
	        <div id="basicform" class="subcontent">
	       	<form class="stdform formvalidate" action="<?php echo U('Admin/Sms/edit');?>" method="post">
                        <p>
                        	<label>企业名称</label>
                            <span class="field">
                            <?php echo ($data["sms_company"]); ?>
                            </span>
                        </p>
                        <?php if(is_array($config)): $i = 0; $__LIST__ = $config;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><p>
                        	<label><?php echo ($key); ?></label>
                            <span class="field">
                            <input type="text" value="<?php echo ($vo); ?>" name="<?php echo ($key); ?>" id="<?php echo ($key); ?>" class="smallinput20" validate='{required:true,messages:{required:"不能为空"}}' />
                            </span>
                        </p><?php endforeach; endif; else: echo "" ;endif; ?>
                        <p class="stdformbutton">
                       	  <input type="hidden" name="sms_id" value="<?php echo ($data["sms_id"]); ?>">
                       		<input type="submit" class="submit radius2 submitedit" value="设置"  />
                        </p>
                    </form>
                    </div>
