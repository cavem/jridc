...
<div class="centercontent tables">
		...
		<div id="contentwrapper" class="contentwrapper">
				<div class="tableoptions">
                	...
                	用户名
					<input type="text" name="username" id="username" value='<?php echo ($username); ?>'  value="" class="smallinput20"/>
					订单编号
					<input type="text" name="ordernumber" id="ordernumber" value='<?php echo ($ordernumber); ?>' value="" class="smallinput20"/>
					开始时间
				...
				<table cellpadding="0" cellspacing="0" border="0" id="table2" class="stdtable stdtablecb">
					...
					<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							...
							<td>
								<?php if($vo["type"] == 1): ?>云主机<?php endif; ?>
								<?php if($vo["type"] == 4): ?>负载均衡<?php endif; ?>
								<?php if($vo["type"] == 5): ?>云硬盘<?php endif; ?>
							</td>
							<td><?php echo ($vo['ordertype']); ?></td>
							<td><?php echo ($vo['usermoney']); ?></td>
							<td>
								<?php if($vo["status"] == 1): ?>未支付<?php endif; ?>
								<?php if($vo["status"] == 2): ?>已支付<?php endif; ?>
								<?php if($vo["status"] == 3): ?>已取消<?php endif; ?>
								<?php if($vo["status"] == 10): ?>已支付(开通失败)<?php endif; ?>
							</td>
							<td><?php echo (date('Y-m-d H:i:s',$vo["addtime"])); ?></td>
							<td class="center">
				 				<a href="<?php echo U('Admin/Order/detail',array('id'=>$vo['id']));?>" class="edit">详情</a> &nbsp;
					 			<a href="<?php echo U('Admin/Order/del',array('id'=>$vo['id']));?>" class="confirmbutton">删除</a>
							</td>
