...
<div class="centercontent tables">
		...
		<div id="contentwrapper" class="contentwrapper">
			<div class="tableoptions">
					<form action="<?php echo U('Admin/Support/index');?>" method="get">
					工单分类
					<select name='type' class="selectwidth20">
						...
						<?php if(is_array($cates)): $i = 0; $__LIST__ = $cates;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value='<?php echo ($vo["id"]); ?>'
							...
							<?php if(is_array($vo["childs"])): $i = 0; $__LIST__ = $vo["childs"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$voo): $mod = ($i % 2 );++$i;?><option value='<?php echo ($voo["id"]); ?>'
								...
								>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo ($voo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endforeach; endif; else: echo "" ;endif; ?>
					</select>
					工单状态
					<select name='status' class="selectwidth20">
						<option value="">--请选择--</option>
						<?php if(is_array($status)): $i = 0; $__LIST__ = $status;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value='<?php echo ($key); ?>'
							<?php if($key == $prm_status): ?>selected<?php endif; ?>
							><?php echo ($vo); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
					</select>
					用户名
					<input type='text' name='username' value='<?php echo ($prm_username); ?>' class='smallinput20'/>
				...
				<table cellpadding="0" cellspacing="0" border="0" id="table2" class="stdtable stdtablecb">
					...
					<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							...
							<td><?php if($vo['type'] == 9999): ?>产品问题<?php else: echo ($vo["cate"]); endif; ?></td>
							<td><?php echo ($vo["username"]); ?></td>
							<td><?php echo ($vo["title"]); ?></td>
							<td class="center">
								<?php if($vo["status"] == 1): ?>已提交<?php endif; ?>
								<?php if($vo["status"] == 2): ?>处理中<?php endif; ?>
								<?php if($vo["status"] == 3): ?>已回复<?php endif; ?>
								<?php if($vo["status"] == 4): ?>追问<?php endif; ?>
								<?php if($vo["status"] == 5): ?>已完成<?php endif; ?>
								<?php if($vo["status"] == 9): ?>已取消<?php endif; ?>
							</td>
							<td class="center"><?php echo (date('Y-m-d H:i:s',$vo["add_time"])); ?></td>
							<td class="center"><?php echo (date('Y-m-d H:i:s',$vo["last_update"])); ?></td>
							<td class="center">
								<a href="<?php echo U('Admin/Support/edit',array('id'=>$vo['id']));?>" class="edit">
								<?php if(($vo["status"] == 5)or($vo["status"] == 9)): ?>查看<?php else: ?>回复<?php endif; ?>
								</a> &nbsp;
					 			<a href="#del" >
					  			<span href="<?php echo U('Admin/Support/del',array('id'=>$vo['id']));?>" class="confirmbutton">删除</span>
					  			</a> &nbsp;
							</td>
