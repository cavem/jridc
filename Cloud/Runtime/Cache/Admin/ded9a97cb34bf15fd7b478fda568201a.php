...
<div class="vernav2 iconmenu">
 ...
 <a class="togglemenu"></a>
		...
		<div class="pageheader notab">
			<h1 class="pagetitle20">添加文章分类</h1>
			<span class="pagedesc">分类添加</span>
		</div><!--pageheader-->
		<div id="contentwrapper" class="contentwrapper">
			<div id="basicform" class="subcontent">
			<form class="stdform formvalidate" action="<?php echo U('Admin/Article/cateadd');?>" method="post">
				<p><label>分类标题</label>
				<span class="field">
				...
				<span class="field">
					<select name='pid' class="selectwidth40-1">
						...
						<?php if(is_array($parents)): $i = 0; $__LIST__ = $parents;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value='<?php echo ($vo["id"]); ?>'><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
					</select>
				</span></p>
				<p><label>分类描述</label>
					<span class="field">
					<textarea class="text-input textarea" id="description" name="description" rows="10" cols="20" ></textarea>
				</span></p>
				<p><label>分类排序</label>
					<span class="field">
					<input type="text" name="sort" id="sort" class="smallinput20" value='0' validate='{required:true,messages:{required:"不能为空"}}' />
				</span></p>
				<p><label>是否启用</label>
				<span class="field">
