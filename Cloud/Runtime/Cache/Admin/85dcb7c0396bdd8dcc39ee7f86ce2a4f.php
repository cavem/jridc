...
<div class="vernav2 iconmenu">
 ...
 <a class="togglemenu"></a>
		...
		<div class="pageheader notab">
			<h1 class="pagetitle20">添加文章</h1>
			<span class="pagedesc">添加文章</span>
		</div><!--pageheader-->
		<div id="contentwrapper" class="contentwrapper">
			<div id="basicform" class="subcontent">
			<form class="stdform formvalidate" action="<?php echo U('Admin/Article/add');?>" method="post">
				<p><label>标题</label>
				<span class="field">
					<input type="text" name="title" id="title" class="smallinput20" validate='{required:true,messages:{required:"不能为空"}}' />
				</span></p>
				<p><label>所属分类</label>
				<span class="field">
					<select name='cid' class="selectwidth40-1">
						<?php if(is_array($cates)): $i = 0; $__LIST__ = $cates;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value='<?php echo ($vo["id"]); ?>'><?php echo ($vo["title"]); ?></option>
							<?php if(is_array($vo["childs"])): $i = 0; $__LIST__ = $vo["childs"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$voo): $mod = ($i % 2 );++$i;?><option value='<?php echo ($voo["id"]); ?>'>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo ($voo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endforeach; endif; else: echo "" ;endif; ?>
					</select>
				</span></p>
				<p><label>文章来源</label>
					<span class="field">
					<input type="text" name="source" id="source" class="smallinput20" value='' />
				</span></p>
				<p><label>描述</label>
